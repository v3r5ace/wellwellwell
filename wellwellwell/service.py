...
def compute_gallons_remaining(percent_full: float | None, full_gallons: float) -> float | None:
    if percent_full is None:
        return None
    return max(0.0, (percent_full / 100.0) * full_gallons)
...
def flush_history(config: AppConfig) -> dict[str, int]:
    with _storage_lock:
        deleted_readings = delete_all_readings(config.db_path)
        removed_files = 0

        if config.snapshots_dir.exists():
            for file_path in config.snapshots_dir.rglob("*"):
    ...
    return {
        ...
        "removed_snapshot_files": removed_files,
    }


def serialize_reading(reading: ReadingRecord | None, config: AppConfig | None = None) -> dict[str, Any] | None:
    if reading is None:
        return None
    ...
    if config is not None:
        payload["gallons_remaining"] = compute_gallons_remaining(reading.percent_full, config.full_gallons)
    return payload
